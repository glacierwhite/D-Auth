...
def init(sd_weights_path, config_path, output_path):
    pretrained_weights = torch.load(sd_weights_path)
    if 'state_dict' in pretrained_weights:
        pretrained_weights = pretrained_weights['state_dict']
    model = create_model(config_path=config_path)
    ...
    target_dict = {}
    for sk in scratch_dict.keys():
        if 'diffusion_model' in sk:
            target_dict[sk] = scratch_dict[sk].clone()
        elif 'local_adapter' in sk:
            target_dict[sk] = scratch_dict[sk].clone()
        elif 'global_adapter' in sk:
            target_dict[sk] = scratch_dict[sk].clone()
        elif sk.replace('text_cond_stage_model.', 'cond_stage_model.') in pretrained_weights.keys():
            target_dict[sk] = pretrained_weights[sk.replace('text_cond_stage_model.', 'cond_stage_model.')].clone()
        else:
            target_dict[sk] = scratch_dict[sk].clone()
            print('new params: {}'.format(sk))
    model.load_state_dict(target_dict, strict=True)
...
if __name__ == '__main__':
    mode = sys.argv[1]
    assert mode in ['init_local', 'init_global', 'init', 'integrate']
    if mode == 'init_local':
