...
class UniControlNet(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        x, c, ct = super().get_input(batch, self.first_stage_key, *args, **kwargs)
        ...
        if len(batch['global_conditions']) != 0:
            ...
            global_condition_weights = batch['global_condition_weights']
            if bs is not None:
                global_conditions = global_conditions[:bs]
                global_condition_weights = global_condition_weights[:bs]
            global_conditions = global_conditions.to(self.device).to(memory_format=torch.contiguous_format).float()
            global_condition_weights = global_condition_weights.to(self.device).to(memory_format=torch.contiguous_format)
        else:
            global_conditions = torch.zeros(1,1).to(self.device).to(memory_format=torch.contiguous_format).float()
            global_condition_weights = torch.zeros(1,1).to(self.device).to(memory_format=torch.contiguous_format)

        return x, dict(c_crossattn=[c], text_cond=[ct], local_control=[local_conditions], global_control=[global_conditions], global_weights=[global_condition_weights])

    def apply_model(self, x_noisy, t, cond, global_strength=1, *args, **kwargs):
        assert isinstance(cond, dict)
        diffusion_model = self.model.diffusion_model
        cond_wav = torch.cat(cond['c_crossattn'], 1)
        cond_text = torch.cat(cond['text_cond'], 1)

        cond_txt = torch.cat([cond_wav, cond_text], dim=1)

        if self.mode in ['global', 'uni']:
            assert cond['global_control'][0] != None
            if cond['global_control'][0].shape[-1] >= 512:
                global_control = self.global_adapter(cond['global_control'][0])
                # global_control = torch.reshape(cond['global_control'][0],(cond['global_control'][0].shape[0],2,512))
                cond_txt = torch.cat([cond_txt, global_strength*global_control], dim=1)
           
        # cond_txt = torch.cat(cond['text_cond'], 1)
